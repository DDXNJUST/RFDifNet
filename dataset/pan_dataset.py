...
class PanDataset(data.Dataset):
    def __init__(
        ...
        norm_range=True,
        full_res=False,
        division=5000, #2047.0,  # don't change
        wavelets=False,
        *,
        ...
        if constrain_channel:
            ...
            self.slice_channel = slice(None)  # slice(None, None, None)

        if not full_res:  #
            # (3000, 4, 256, 256), (3000, 4, 64, 64), (3000, 4, 256, 256), (3000, 1, 256, 256)
            # self.gt, self.ms, self.lms, self.pan = self.get_divided(d)
            # self.gt, self.ms, self.lms, self.pan, self.unalignedpan = self.get_divided(d)
            self.gt, self.ms, self.pan, self.unalignedpan = self.get_divided(d)

            # if wavelets:
            #     print("processing wavelets...")
            #     # 小波变换 1）低频系数，以数组形式存放。2）高频系数，每一层的（水平、垂直、对角线）高频系数构成一个 3 维元组
            ...
            # print("{:^20}{:^20}{:^20}{:^20}{:^20}".format("pan", "ms", "lms", "gt", "unalignedpan"))
            print("{:^20}{:^20}{:^20}{:^20}".format("pan", "ms", "gt", "unalignedpan"))
            print(
                "{:^20}{:^20}{:^20}{:^20}".format(
                    str(self.pan.shape),
                    str(self.ms.shape),
        ...
        self.size = self.ms.shape[0]  # 3000

        # highpass filter
        self.hp = hp  # False
        self.hp_ksize = hp_ksize  # (5, 5)
        ...
        self.unalignedpan = norm_func(self.unalignedpan)
        self.ms = norm_func(self.ms)
        # self.lms = norm_func(self.lms)
        if wavelets:
            # torch.Size([3000, 7, 128, 128])
        ...
        self.aug_prob = aug_prob  # 0
        self.random_erase_channel = RandomEraseChannel(self.ms.shape[1])  # lms
        self.geo_trans = (
            T.Compose(
    ...
    def get_divided(self, d, full_resolution=False):
        if not full_resolution:
            ...
            return (
                ...
                np.asarray(d["unalignedpan"], dtype=float),
            )
        else:
            return (
                np.asarray(d["ms"], dtype=float)[:, self.slice_channel],
    ...
    def __getitem__(self, item):
        if hasattr(self, "gt"):
            if not self.wavelets:
                # tuple_data = (self.pan[item], self.lms[item], self.gt[item], self.unalignedpan[item])
                tuple_data = (self.pan[item], self.ms[item], self.gt[item], self.unalignedpan[item])
            else:
                tuple_data = (
                    self.pan[item],
                    self.lms[item],
