...
class UNetSR3(nn.Module):
    ...
    def forward(self, x, time, cond=None, self_cond=None): # torch.Size([20, 8, 256, 256]), torch.Size([20])
        # pdb.set_trace()
        # self-conditioning
        if self.self_condition:
            self_cond = default(self_cond, x)
            x = torch.cat([self_cond, x], dim=1)  # torch.Size([20, 16, 256, 256])
        ...
        t = self.noise_level_mlp(time) if exists(self.noise_level_mlp) else None  # torch.Size([20, 32])
