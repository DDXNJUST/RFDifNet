...
class UNetSR3(nn.Module):
    def __init__(
        ...
        if fourier_features:
            ...
            in_channel = in_channel + in_channel * n * 2
        ...
        # downs = [nn.Conv2d(9, inner_channel, kernel_size=3, padding=1)]
        # pdb.set_trace()
        for ind in range(num_mults):  # 4
            # print(now_res, attn_res)
            is_last = ind == num_mults - 1
            # use_attn = now_res in attn_res
            use_attn = False
            channel_mult = inner_channel * channel_mults[ind]  # (1, 2, 4, 8, 8)
            for _ in range(0, res_blocks):  # 3
                downs.append(
                    ResnetBlocWithAttn(
                        pre_channel,
                        channel_mult,
    ...
    def forward(self, x, time, cond=None, self_cond=None):  # torch.Size([2, 4, 256, 256]), torch.Size([2]), torch.Size([2, 5, 256, 256]), None
        # self-conditioning 2*res
        # pdb.set_trace()
        if self.self_condition:  #
            self_cond = default(self_cond, x)  # torch.Size([2, 4, 256, 256])
            x = torch.cat([self_cond, x], dim=1)  # torch.Size([2, 8, 256, 256])
        # x = torch.cat([x, cond], dim=1)  # torch.Size([2, 8, 256, 256])

        t = self.noise_level_mlp(time) if exists(self.noise_level_mlp) else None  # 时序编码 torch.Size([2, 32])
        ...
        for layer in self.downs:
            ...
            if isinstance(layer, ResnetBlocWithAttn):
                # x = layer(x, t, cond[:, : self.lms_channel + self.pan_channel])  # cond: cat[lms, pan]
                x = layer(x, t, cond)
                # x = layer(x, t)
            else:
                x = layer(x)  #
...
class ResnetBlock(nn.Module):  # RegBlock
    def __init__(
        self,
        dim,
        dim_out,
...
class ResnetBlocWithAttn(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.res_block = ResnetBlock(
            dim_out if exists(cond_dim) else dim,  # 条件引导 dim+5 if exists(cond_dim) else dim,  #
            dim_out,
            noise_level_emb_dim,
            norm_groups=norm_groups,
            dropout=dropout,
    ...
    def forward(self, x, time_emb, cond=None):  # torch.Size([2, 32, 256, 256]), torch.Size([2, 32]), torch.Size([2, 5, 256, 256])
        # condition injection
        if self.with_cond:  # 条件引导
            x = self.cond_inj(
                x, F.interpolate(cond, size=x.shape[-2:], mode="bilinear")
            )
            # x= torch.cat([x, F.interpolate(cond, size=x.shape[-2:], mode="bilinear")], dim=1)
        # pdb.set_trace()
