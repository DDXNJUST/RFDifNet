    ...
    def __init__(
        ...
        for ind in range(num_mults):
            ...
            for _ in range(0, res_blocks):
                downs.append(ResnetBlocWithAttn_reg(
        ...
        self.final_conv_defm_reg = Block_reg(pre_channel+1,2,groups=3)  # dim_out = 3  # 原始
        # self.final_conv_defm_reg = Block_reg(pre_channel + 4, 2, groups=3)
        # self.final_conv_defm_reg = Block_reg(1,2,groups=1)  # dim_out = 3
    ...
    def forward(self, x, x_m, time):  # torch.Size([2, 3, 256, 256]) torch.Size([2, 1, 256, 256]) torch.Size([2])
        ...
        for layerd,layerr,layera in zip(self.ups_diff, self.ups_regis, self.ups_adapt):
            if isinstance(layerd, ResnetBlocWithAttn_reg):
                # print('up, attention')
                feat=feats.pop()
                x_1 = layerd(torch.cat((x_1, feat), dim=1), t)  # 原始
                x_2 = layerr(torch.cat((x_2, feat,x_1), dim=1), t)  # 原始
